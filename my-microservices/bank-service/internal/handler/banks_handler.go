...
import (
	...
	"my-microservices/bank-service/internal/service"
...
func (h *BanksHandler) Create() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var payload domain.Bank
		if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
			h.logger.Error(domain.ErrInvalidJsonFormat.Error(), zap.Error(err))
			span.RecordError(err)
			dto.WriteError(
				w,
				domain.StatusCodeHandler(domain.ErrInvalidJsonFormat),
				strconv.Itoa(domain.StatusCodeHandler(domain.ErrInvalidJsonFormat)),
				domain.ErrInvalidJsonFormat.Error(),
			)
			return
		}
		...
		if err := h.rdb.Del(ctx, cacheKey).Err(); err != nil {
			...
			span.RecordError(err)
			span.SetStatus(codes.Error, domain.ErrRedisInvalidate.Error())
...
func (h *BanksHandler) Update() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			dto.WriteError(
				w,
				domain.StatusCodeHandler(domain.ErrInvalidUuid),
				strconv.Itoa(domain.StatusCodeHandler(domain.ErrInvalidUuid)),
				domain.ErrInvalidUuid.Error(),
			)
			return
		}
